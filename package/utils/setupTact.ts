...
export default async function setupTact(
  targetDir: string,
  projectName: string
) {
  ...
  execSync(
    "npm install @ton/sandbox @ton/test-utils @types/jest @types/node jest  @ton/ton @ton/core @ton/crypto --save-dev",
    { stdio: "inherit" }
  );
  ...
  const tactTemplatePath = path.join(
    dirname(fileURLToPath(import.meta.url)),
    "../fileTemplates/TactTemplate.tact"
  );
